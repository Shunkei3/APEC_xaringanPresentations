...
.content-box-green[Example 3: Utility maximization]
...
```{r, cache=FALSE, fig.dim = c(6, 5), out.width="70%"}
...
`r fa('skull',height='20px',fill='black')`: Creating or modifying or CSS file is a daunting task... 
...
There are lots of cool xaringan slides out there!

.panelset[ 
<!-- Panel 1: Panelset-->
.panel[.panel-name[UW–Madison]

Press `o` key on the slide below. 
```{r, fig.dim = c(4, 6), out.width = "50%"}
embed_xaringan(
  url = "https://ttuowang.github.io/Xaringan-wisconsin-theme/#1"
)
```
  ]
<!-- Panel 2: Panelset -->
.panel[.panel-name[Rutgers U]
```{r, fig.dim = c(4, 6), out.width = "50%"}
embed_xaringan(
  url = "https://www.jvcasillas.com/ru_xaringan/slides/index.html#1"
)
```
  ]
<!-- Panel 3: Panelset -->
.panel[.panel-name[Some other]
```{r, fig.dim = c(4, 6), out.width = "50%"}
embed_xaringan(
  url = "https://slides.silviacanelon.com/2021-03-23-amia-informatics-summit/#1"
)
```
  ]
]
...
WebR package + Quarto revealjs slides
